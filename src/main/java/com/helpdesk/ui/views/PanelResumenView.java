...
public class PanelResumenView extends BorderPane {
    ...
    public void cargar() {
        ...
        Map<Prioridad, Long> resumen = service.resumenAbiertasPorPrioridad();
        HBox tarjetas = new HBox(16);
        tarjetas.setAlignment(Pos.CENTER_LEFT);

        for (Prioridad p : Prioridad.values()) {
            long count = resumen.getOrDefault(p, 0L);
        ...
        // Total
        long total = resumen.values().stream().mapToLong(Long::longValue).sum();
